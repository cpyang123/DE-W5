import argparse
from mylib.lib import create_connect_database, delete_record, load_dataframe, load_to_table, select_all, insert_house_price_record, select_id
...
read_parser.add_argument('--id', type=int, help='ID of the record to read (leave empty to read all)')

# 4. Create 'delete_record' command
delete_parser = subparsers.add_parser('delete_record', help='Delete a record')
delete_parser.add_argument('--id', type=int, required=True, help='ID of the record to delete')
...
if args.command == 'init':
    ...
    insert_house_price_record(args.MedInc, args.HouseAge, args.AveRooms, args.AveBedrms, args.Population, args.AveOccup, args.Latitude, args.Longitude, args.MedHouseVal)
    print(f"Creating record: Medinc = {args.MedInc}, HouseAge = {args.HouseAge}")

elif args.command == 'update_record':
    # Handle record update
    engine = create_connect_database("demo.db")
    update_house_price_record(args.id, args.MedInc, args.HouseAge, args.AveRooms, args.AveBedrms, args.Population, args.AveOccup, args.Latitude, args.Longitude, args.MedHouseVal)
